...
func play_animation(movement):
	...
	if dir == "right":
		...
		if movement == 1:
			animation.play("player_lightning_mage_run")
		elif movement == 0:
			animation.play("player_lightning_mage_idle")
...
func attack():
	...
	if Input.is_action_just_released("attack"):
		...
		attack_ip = false
		$AnimatedSprite2D.play("player_lightning_mage_idle")  # Reset to idle animation		
