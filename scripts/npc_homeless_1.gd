...
const IDLE_DURATION_3 = 2.5  # Duration for npc_homeless_idle3
...
func move_npc(delta):
    $AnimatedSprite2D.play("npc_homeless_walk")  # Play walking animation
...
func enter_idle_state():
    ...
    var random_idle = randi() % 3
    var idle_animation = ""
    
    if random_idle == 0:
        idle_animation = "npc_homeless_idle1"
        idle_duration = IDLE_DURATION_1  # Set idle duration for idle1
    elif random_idle == 1:
        idle_animation = "npc_homeless_idle2"
        idle_duration = IDLE_DURATION_2  # Set idle duration for idle2
    elif random_idle == 2:
        idle_animation = "npc_homeless_idle3"
        idle_duration = IDLE_DURATION_3  # Set idle duration for idle3
